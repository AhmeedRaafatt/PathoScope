...
// Main loader for Hematology module
export async function loader() {
    const token = requireAuth()
    const base = 'http://127.0.0.1:8000/api/hematology'
    const headers = {
    ...
    try {
        const [scheduledRes, dashboardRes, queueRes, analytesRes] = await Promise.all([
            fetch(`${base}/scheduled-patients/`, { headers }),
            fetch(`${base}/dashboard/`, { headers }),
            fetch(`${base}/queue/`, { headers }),
            fetch(`${base}/analytes/`, { headers })
        ])

        // Check for auth errors
        const allResponses = [scheduledRes, dashboardRes, queueRes, analytesRes]
        if (allResponses.some(r => r.status === 401 || r.status === 403)) {
            localStorage.removeItem('token')
            throw redirect('/login')
        }

        const [scheduledPatients, samples, queue, analytes] = await Promise.all([
